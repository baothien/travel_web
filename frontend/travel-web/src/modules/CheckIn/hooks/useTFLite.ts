...
function useTFLite() {
  ...
  useEffect(() => {
    async function loadTFLiteModel() {
      if (!tflite || (isSIMDSupported && !tfliteSIMD)) {
        return
      }

      setSelectedTFLite(undefined)

      const newSelectedTFLite = tfliteSIMD
      ...
      console.log(`${process.env.PUBLIC_URL}/models/${modelFileName}.tflite`)
    ...
    loadTFLiteModel()
  }, [tflite, tfliteSIMD, isSIMDSupported])
